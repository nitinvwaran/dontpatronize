...
class ModelRoberta(nn.Module):
    ...
    def forward(self,dataframe):
        ...
        inp = self.bertmodel.tokenizer(sentences, max_length=self.maxlen, padding='max_length', truncation=True,return_tensors='pt')
        inp.to(self.device)
        #inp['output_hidden_states'] = True
        #attentionmask = inp['attention_mask']
...
class TrainEval():
    ...
    def train_eval_bert(self,labeltype):
...
def main():
    ...
    #traineval.train_eval_bert(labeltypes[0])
